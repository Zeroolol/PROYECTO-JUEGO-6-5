...
func interact():
	...
	if interactable:
		...
		if opened:
			$AnimationPlayer.play("AbrirPuerta2")  # Si la puerta está abierta, reproduce la animación de abrir
		else:
			$AnimationPlayer.play("CerrarPuerta2")  # Si la puerta está cerrada, reproduce la animación de cerrar
